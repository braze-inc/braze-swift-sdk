...
- (void)viewDidLoad {
  ...
  */
  ...
  if (((ABKInAppMessageHTMLBase *)self.inAppMessage).assetsLocalDirectoryPath != nil) {
    ...
    [fileManager createDirectoryAtPath:localPath.path withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    NSURL *html = [localPath URLByAppendingPathComponent:ABKInAppMessageHTMLFileName];
    [self.inAppMessage.message writeToURL:html atomically:NO encoding:NSUTF8StringEncoding error:nil];
    if (![fileManager fileExistsAtPath:html.path]) {
      NSLog(@"Can't find HTML at path %@, with file name %@. Aborting display.", localPath, ABKInAppMessageHTMLFileName);
      [self hideInAppMessage:NO];
    }
    [self.webView loadFileURL:html allowingReadAccessToURL:localPath];
  } else {
...
decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
  ...
  BOOL isIframeNavigation = navigationAction.targetFrame != nil && navigationAction.targetFrame.mainFrame == NO;
